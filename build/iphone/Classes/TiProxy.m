...
//Rare exceptions to indicate a bug in the _everettsinterface code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_everettsinterface = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_everettsinterface]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.everettsinterface.module"
